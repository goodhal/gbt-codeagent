...
> - [漏洞类型详解](vulnerabilities/) - SQL注入、命令注入等漏洞检测方法
...
详细分工说明见 [audit_workflow.md](workflow/audit_workflow.md)「三层审计分工」章节。

| 分工 | 负责方 | 发现的漏洞类型 |
|------|--------|---------------|
| **快速扫描** | 代码（正则表达式） | 高风险函数调用 |
| **LLM审计** | LLM（语义分析） | 需要上下文分析的漏洞 |
| **LLM审查** | LLM（深入分析） | 复杂业务逻辑、漏洞验证 |
...
> **国标映射规则**：
> - **GB/T 39412-2020 是通用基线，所有语言都必须加载**
> - 有专用标准的语言（Java/C/C++/C#）使用双映射：`专用标准；GB/T39412-规则`
> - 无专用标准的语言（Python/Go/JS/PHP/Rust 等）使用单映射：`GB/T39412-规则`
...
> - 无专用标准的语言（Python/Go/JS/PHP/Rust 等）：单映射，格式 `GB/T39412-{规则}`
> - 分隔符使用中文分号 `；`
