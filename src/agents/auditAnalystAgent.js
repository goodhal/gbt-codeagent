...
export class AuditAnalystAgent {
  ...
  async run({ taskId, projects, selectedSkillIds, llmConfig, useReAct = false, useStreaming = false, reactConfig = {}, enableLlmAudit = true, onProgress, shouldCancel, tasks, onProjectGroupComplete }) {
    ...
    const validatedResults = await Promise.all(results.map(async (result) => {
      ...
      // 统一修正所有发现的必填字段（CWE/OWASP/GB/T映射/CVSS等）
      for (const f of validated) {
        enrichFindingFields(f);
      }
...
请仅输出精简JSON（每个发现只填4个字段，减少token消耗）：{"findings":[{"title":"漏洞名称(15字内)","severity":"critical|high|medium|low","location":"文件:行号","vulnType":"XSS|SQL_INJECTION|..."}]}`;
    ...
    // ======== 安全性网：补齐未被 LLM 覆盖的高危 QuickScan 发现 ========
    const covered = new Set();
    for (const vr of validatedResults) {
      for (const f of vr.findings) {
        if (f.source === 'llm' || f.skillId === 'vuln-type-gapfill') {
          const file = (f.location || f.file || '').split(':')[0];
          covered.add(file + '::' + f.vulnType);
        }
      }
    }
    for (const vr of validatedResults) {
      for (const f of vr.findings) {
        if (f.source !== 'quick_scan') continue;
        const sev = (f.severity || f.severityLabel || '').toLowerCase();
        if (sev !== 'critical' && sev !== 'high') continue;
        const file = (f.location || f.file || '').split(':')[0];
        const key = file + '::' + f.vulnType;
        if (!covered.has(key)) {
          // LLM 未覆盖的 critical/high QuickScan 发现，标记来源并保留
          f._llmUnverified = true;
        }
      }
    }
...
confirmed为true表示确认为真实漏洞，false表示为误报。仅确认真实漏洞，不要添加新发现。`;
    ...
    try {
      ...
      for (const v of verified) {
        ...
        validatedResults[0].findings.push(enrichFindingFields(newFinding));
        confirmedCount++;
      }
...
function enrichFindingFields(f) {
  ...
  // GB/T 映射 — 数据已搬迁至 config/gbt-mappings.yaml
  // quickScanService 已从 YAML 读取并设置 f.gbtMapping；这里是兜底
  if (!f.gbtMapping || f.gbtMapping === '' || f.gbtMapping === 'unknown' || f.gbtMapping === 'undefined' || f.gbtMapping === '无' || f.gbtMapping === 'GB/T39412-2020 通用基线') {
    // 优先使用 vulnType→具体条款映射，避免降级到通用基线
    const GBT_FALLBACK_MAP = {
      'SQL_INJECTION': 'GB/T39412-8.3.2 SQL注入',
      'SQL_INJECTION_MYBATIS': 'GB/T39412-8.3.2 SQL注入',
      'SQL_INJECTION_ORDERBY': 'GB/T39412-8.3.2 SQL注入',
      'SQL_INJECTION_GROUPBY': 'GB/T39412-8.3.2 SQL注入',
      'SQL_INJECTION_HQL': 'GB/T39412-8.3.2 SQL注入',
      'COMMAND_INJECTION': 'GB/T39412-6.1.1.6 命令行注入',
      'CODE_INJECTION': 'GB/T39412-7.3.6 暴露危险的方法或函数',
      'SPEL_INJECTION': 'GB/T39412-7.3.6 暴露危险的方法或函数',
      'SSTI': 'GB/T39412-7.3.6 暴露危险的方法或函数',
      'JNDI_INJECTION': 'GB/T39412-7.3.6 暴露危险的方法或函数',
      'DESERIALIZATION': 'GB/T39412-7.1.5 存储不可序列化的对象到磁盘',
      'PATH_TRAVERSAL': 'GB/T39412-6.1.1.1 输入验证不足',
      'ARBITRARY_FILE_READ': 'GB/T39412-7.3.3 路径遍历',
      'FILE_READ': 'GB/T39412-6.1.1.1 输入验证不足',
      'FILE_UPLOAD': 'GB/T39412-6.1.1.1 输入验证不足',
      'UNRESTRICTED_FILE_UPLOAD': 'GB/T39412-6.1.1.1 输入验证不足',
      'SSRF': 'GB/T39412-6.1.1.1 输入验证不足',
      'XSS': 'GB/T39412-6.1.2.1 跨站脚本(XSS)攻击',
      'XSS_REFLECTED': 'GB/T39412-6.1.2.1 跨站脚本(XSS)攻击',
      'XSS_STORED': 'GB/T39412-6.1.2.1 跨站脚本(XSS)攻击',
      'XXE': 'GB/T39412-6.1.1.1 输入验证不足',
      'XXE_INJECTION': 'GB/T39412-6.1.1.1 输入验证不足',
      'HARDCODED_CREDENTIALS': 'GB/T39412-6.2.1.3 使用安全相关的硬编码',
      'HARD_CODE_PASSWORD': 'GB/T39412-6.2.1.3 使用安全相关的硬编码',
      'PLAINTEXT_PASSWORD': 'GB/T39412-6.2.1.3 使用安全相关的硬编码',
      'WEAK_CRYPTO': 'GB/T39412-6.2.1.1 密码安全不符合国密管理规定',
      'WEAK_HASH': 'GB/T39412-6.2.1.1 密码安全不符合国密管理规定',
      'RSA_WEAK_PADDING': 'GB/T39412-6.2.1.1 密码安全不符合国密管理规定',
      'PREDICTABLE_RANDOM': 'GB/T39412-6.2.1.2 随机数安全',
      'WEAK_RANDOM': 'GB/T39412-6.2.1.2 随机数安全',
      'AUTH_BYPASS': 'GB/T39412-6.3.1.2 身份鉴别被绕过',
      'REFERER_AUTH_BYPASS': 'GB/T39412-6.3.1.2 身份鉴别被绕过',
      'AUTH_BYPASS_URI': 'GB/T39412-6.3.1.2 身份鉴别被绕过',
      'AUTH_BYPASS_SUFFIX': 'GB/T39412-6.3.1.2 身份鉴别被绕过',
      'AUTH_BYPASS_SPRING': 'GB/T39412-6.3.1.2 身份鉴别被绕过',
      'AUTH_INFO_EXPOSURE': 'GB/T39412-6.3.1.1 身份鉴别过程暴露多余信息',
      'MISSING_ACCESS_CONTROL': 'GB/T39412-6.3.3.1 不安全的直接对象引用',
      'IDOR': 'GB/T39412-6.3.3.1 不安全的直接对象引用',
      'CSRF': 'GB/T39412-6.3.3.2 跨站请求伪造',
      'CSRF_MISSING': 'GB/T39412-6.3.3.2 跨站请求伪造',
      'CSRF_DISABLED': 'GB/T39412-6.3.3.2 跨站请求伪造',
      'CSRF_PROTECTION': 'GB/T39412-6.3.3.2 跨站请求伪造',
      'AUTH_CSRF_DISABLED': 'GB/T39412-6.3.3.2 跨站请求伪造',
      'SESSION_FIXATION': 'GB/T39412-7.2.1 不同会话间信息泄露',
      'INFO_LEAK': 'GB/T39412-6.2.2.1 敏感信息暴露',
      'INFORMATION_DISCLOSURE': 'GB/T39412-6.2.2.1 敏感信息暴露',
      'EXCEPTION_INFO_LEAK': 'GB/T39412-6.2.2.1 敏感信息暴露',
      'INSECURE_COOKIE_AUTH': 'GB/T39412-6.2.2.1 敏感信息暴露',
      'COOKIE_MANIPULATION': 'GB/T39412-6.2.2.1 敏感信息暴露',
      'SWAGGER_EXPOSURE': 'GB/T39412-6.2.2.1 敏感信息暴露',
      'PLAINTEXT_TRANSMISSION': 'GB/T39412-6.2.2.1 敏感信息暴露',
      'LOG_INJECTION': 'GB/T39412-6.4.1 对输出日志中特殊元素处理',
      'BUFFER_OVERFLOW': 'GB/T39412-8.2.6 内存缓冲区边界操作越界',
      'UNCONTROLLED_MEMORY': 'GB/T39412-8.2.6 内存缓冲区边界操作越界',
      'FORMAT_STRING': 'GB/T39412-7.3.1 格式化字符串',
      'FORMAT_STRING_VULNERABILITY': 'GB/T39412-7.3.1 格式化字符串',
      'PROCESS_CONTROL': 'GB/T39412-7.3.6 暴露危险的方法或函数',
      'INTEGER_OVERFLOW': 'GB/T39412-6.1.1.12 数值赋值越界',
      'OPEN_REDIRECT': 'GB/T39412-6.1.2.3 URL重定向向不可信站点',
      'CORS_MISCONFIGURATION': 'GB/T39412-6.3.2.2 跨域资源共享配置不当',
      'RACE_CONDITION': 'GB/T39412-7.2.3 共享资源的并发安全',
      'NO_RATE_LIMIT': 'GB/T39412-6.3.1.3 登录失败处理不当',
      'XPATH_INJECTION': 'GB/T39412-6.1.1.1 输入验证不足',
      'COMPONENT_VULNERABILITY': 'GB/T39412-6.1.1.5 使用已知脆弱组件',
      'VULNERABLE_DEPENDENCY': 'GB/T39412-6.1.1.5 使用已知脆弱组件',
      'STRUTS_WILDCARD': 'GB/T39412-6.1.1.5 使用已知脆弱组件',
      'IMPROPER_EXCEPTION_HANDLING': 'GB/T39412-7.4.1 异常处理不当',
      'ASSERT_MISUSE': 'GB/T39412-7.4.1 异常处理不当',
      'INFINITE_LOOP': 'GB/T39412-8.1.8 无限循环',
      'BLACKLIST_VALIDATION': 'GB/T39412-6.1.1.1 输入验证不足',
      'WEAK_PASSWORD_POLICY': 'GB/T39412-6.2.1.3 使用安全相关的硬编码',
      'REGEX_DENIAL_OF_SERVICE': 'GB/T39412-6.1.1.1 输入验证不足',
      'HTTP_RESPONSE_SPLITTING': 'GB/T39412-6.1.1.1 输入验证不足',
    };
    const fallbackCode = GBT_FALLBACK_MAP[vulnType];
    f.gbtMapping = fallbackCode || 'GB/T39412-2020 通用基线';
  }
  ...
  if (!f.remediation || f.remediation.trim() === '') {
    ...
    if (sev === 'critical') {
      f.remediation = '立即修复：对' + (f.vulnType || '该漏洞') + '进行安全加固，添加输入验证、参数化查询或白名单校验；如无法立即修复，应添加访问控制限制';
    } else if (sev === 'high') {
      f.remediation = '尽快修复：对' + (f.vulnType || '该漏洞') + '进行安全加固，参考 OWASP 或 GB/T 标准中对应的修复建议';
    } else {
      f.remediation = '建议修复：对' + (f.vulnType || '该漏洞') + '进行安全加固，遵循最小权限原则和安全编码规范';
    }
  }

  // safeValidation 兜底
  if (!f.safeValidation || f.safeValidation.trim() === '' || f.safeValidation === '建议人工复核代码上下文，确认是否存在实际安全风险') {
    f.safeValidation = '建议人工复核代码上下文，确认是否存在实际安全风险；可通过发送测试 payload 验证漏洞是否确实可利用';
  }
