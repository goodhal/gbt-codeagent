...
export class TaintFlowTracker {
  ...
  addSource(variable, sourceInfo) {
    this.sources.set(variable, sourceInfo);
    this.taintedVariables.add(variable);
  }

  addSanitizer(variable, sanitizerInfo) {
    this.sanitizers.set(variable, sanitizerInfo);
    this.taintedVariables.delete(variable);
  }

  isTainted(variable) {
    return this.taintedVariables.has(variable);
  }

  getSource(variable) {
    return this.sources.get(variable);
  }
  ...
  getFlow(variable) {
    ...
    while (current && this.sources.has(current)) {
      flow.unshift({
        variable: current,
        source: this.sources.get(current)
      });
      current = this.sources.get(current)?.parent;
    }
