...
// GBT_MAPPING 已搬迁至 config/gbt-mappings.yaml
// 通过 rulesEngine.getGbtMapping(vulnType, language) 获取
...
export class QuickScanService {
  constructor() {
    ...
    // gbtMapping 已搬迁至 config/gbt-mappings.yaml，通过 _rulesEngine.getGbtMapping 获取
  ...
  async _ensureEngine() {
    ...
    this._engineInitPromise = (async () => {
      try {
        this._rulesEngine = await getRulesEngine('./config');
  ...
  getGbtMapping(vulnType, language) {
    // 优先从 rulesEngine 获取（YAML 已搬迁至 config/gbt-mappings.yaml）
    if (this._rulesEngine && typeof this._rulesEngine.getGbtMapping === 'function') {
      const result = this._rulesEngine.getGbtMapping(vulnType, language);
